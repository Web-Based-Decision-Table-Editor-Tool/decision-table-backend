...
And I have created decision table named <dec_name> for querying rule
And I have a condition in this table named <con_name> with type <con_type> and values <con_vals> for querying rule
And I have an action in this table of type <action_type> named <action_name> with action values: <action_values> for querying rule
And I have created a rule in this table with condition <con_name> and condition value <con_num>, and action <action_name> and action value <act_num> for querying rule
When I query the created rule with its unique id
Then the recieved rule is identical to the previously created rule
And I receive an error code as <response_code> for querying rule
